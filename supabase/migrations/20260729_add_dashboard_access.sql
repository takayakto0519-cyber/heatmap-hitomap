-- 自治体・法人向け集計ダッシュボードの顧客専用アクセストークン（Phase 2）
-- 顧客はSupabase Authのアカウントを持たず、発行されたトークン付きURLでのみ
-- 自分の地域の集計データ（app/api/dashboard/[token]）を閲覧できる。
CREATE TABLE IF NOT EXISTS dashboard_access (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  client_lead_id uuid REFERENCES client_leads(id) ON DELETE CASCADE,
  token text NOT NULL UNIQUE,
  region text NOT NULL,       -- traces.region と一致させる集計対象の地域名
  label text,                 -- ダッシュボード上に出す表示名（例：「〇〇市役所様」）
...
-- /api/migrate から呼ばれる自動マイグレーション関数に反映（既存の冪等マイグレーション関数を再定義。全カラム定義を維持したまま追記）
