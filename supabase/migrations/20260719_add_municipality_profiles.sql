-- 自治体プロファイル（municipality_profiles）：関係人口ダッシュボードの一部として、
-- 営業対象・実証先の自治体ごとに「調べた内容」「関係人口創出の取り組みの有無」
-- 「ヒトマップとの親和性」「提案余地」をまとめて残せるようにする。
-- engagement_stage: observing(観察) | lead(リード) | proposed(提案中) | contracted(契約済み)
-- opportunity_level: 高 | 中 | 低

CREATE TABLE IF NOT EXISTS municipality_profiles (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  region_name text NOT NULL UNIQUE,
  engagement_stage text NOT NULL DEFAULT 'lead',
  evidence_summary text,
  relation_population_initiative text,
  fit_assessment text,
  opportunity_level text NOT NULL DEFAULT '中',
  opportunity_notes text,
  source_links text,
  created_at timestamptz DEFAULT now(),
  updated_at timestamptz DEFAULT now()
);
CREATE INDEX IF NOT EXISTS idx_municipality_profiles_stage ON municipality_profiles (engagement_stage);

-- ensure_schema() の再定義（/api/migrate から呼ばれる冪等マイグレーション関数。全定義を維持したまま追記）
-- 【重要】このファイルは直前の20260719_extend_ai_ops_and_pnl.sqlをベースに、municipality_profilesのみ追記している。
...
BEGIN
  ...
  CREATE TABLE IF NOT EXISTS revenue_initiatives (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    title text NOT NULL,
    stage text NOT NULL DEFAULT '未着手',
    next_action text,
    created_at timestamptz DEFAULT now(),
    updated_at timestamptz DEFAULT now()
  );
  ALTER TABLE revenue_initiatives ADD COLUMN IF NOT EXISTS code text;
  CREATE INDEX IF NOT EXISTS idx_revenue_initiatives_stage ON revenue_initiatives (stage);

  CREATE TABLE IF NOT EXISTS business_cases (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    org_name text NOT NULL,
    client_type text NOT NULL DEFAULT 'business',
    stage text NOT NULL DEFAULT '発案',
    evidence text,
    proposal_link text,
    next_action text,
    lead_ref uuid REFERENCES client_leads(id) ON DELETE SET NULL,
    created_at timestamptz DEFAULT now(),
    updated_at timestamptz DEFAULT now()
  );
  CREATE INDEX IF NOT EXISTS idx_business_cases_stage ON business_cases (stage);
  ...
  CREATE TABLE IF NOT EXISTS line_bot_settings (
    ...
    group_id text,
    mission_interval_days integer NOT NULL DEFAULT 14,
    auto_push boolean NOT NULL DEFAULT false,
    auto_welcome boolean NOT NULL DEFAULT false,
    members jsonb NOT NULL DEFAULT '[]',
    updated_at timestamptz DEFAULT now()
  );

  CREATE TABLE IF NOT EXISTS sales_email_targets (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  CREATE INDEX IF NOT EXISTS idx_sales_email_targets_sent ON sales_email_targets (sent);

  CREATE TABLE IF NOT EXISTS business_line_pnl (
    id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    created_at timestamptz DEFAULT now(),
    updated_at timestamptz DEFAULT now(),
    UNIQUE (line_key, month)
  );
  CREATE INDEX IF NOT EXISTS idx_business_line_pnl_month ON business_line_pnl (month DESC);
