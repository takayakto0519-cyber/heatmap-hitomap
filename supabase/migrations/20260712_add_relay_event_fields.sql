-- リレー型イベント（例：山手線一周プロジェクト）：事前に決まった地点ではなく、参加者がその場で発見して投稿する形式
ALTER TABLE routes ADD COLUMN IF NOT EXISTS event_mode text DEFAULT 'route';   -- 'route'（従来の事前ルート型） | 'relay'（発見連鎖型）
ALTER TABLE routes ADD COLUMN IF NOT EXISTS event_session_code text;           -- relay型イベントの参加者投稿を束ねるsession_code
ALTER TABLE traces ADD COLUMN IF NOT EXISTS team text;                         -- relay参加時のチーム名（自由記述、nicknameと同じ運用）
