-- おすすめルート公開＋運営承認：ログインユーザーがルートを「おすすめルート」として公開申請できるようにする
ALTER TABLE routes ADD COLUMN IF NOT EXISTS is_public_recommendation boolean DEFAULT false;
ALTER TABLE routes ADD COLUMN IF NOT EXISTS review_status text;   -- null | 'pending' | 'approved' | 'rejected'
ALTER TABLE routes ADD COLUMN IF NOT EXISTS highlights text;      -- 見どころ・おすすめポイント（自由記述）
