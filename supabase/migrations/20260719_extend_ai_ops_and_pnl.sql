-- AIエージェント運営タブの入力しやすさ改善＋「事業別損益(P&L)」の追加。
-- 20260718_add_ai_ops_tables.sql をベースに、以下を追加する：
--   1) revenue_initiatives に施策コード列（例：B-1）を追加
--   2) business_line_pnl テーブルを新設（顧問・キット販売・SaaS・採用商材等、事業ラインごとの月次損益）
-- 既存の運用（Supabase SQL Editorで手動適用）に合わせ、直接適用文とensure_schema()の両方に反映する。

ALTER TABLE revenue_initiatives ADD COLUMN IF NOT EXISTS code text;

CREATE TABLE IF NOT EXISTS business_line_pnl (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  line_key text NOT NULL,   -- advisory(顧問業) | kit(キット販売) | saas(SaaS) | recruit(採用商材) | event(イベント) | other(その他)
  month date NOT NULL,      -- 集計月（その月の1日で保存する）
