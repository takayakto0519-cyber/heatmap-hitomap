-- municipality_profiles に SMOUT（smout.jp）経由の営業アプローチを記録する列を追加する。
-- SMOUTには公式APIが無くgmail_watch.pyのような自動検知はできないため、
-- email_sent_at/email_replyと同じ発想の「手動記録」列にする（会長がダッシュボードで手で更新）。
ALTER TABLE municipality_profiles ADD COLUMN IF NOT EXISTS smout_sent_at timestamptz;
ALTER TABLE municipality_profiles ADD COLUMN IF NOT EXISTS smout_reply text;

-- /api/migrate から呼ばれる自動マイグレーション関数にも反映（全定義を維持したまま追記）
...
BEGIN
  ...
  ALTER TABLE municipality_profiles ADD COLUMN IF NOT EXISTS followed_up_at timestamptz;
  ALTER TABLE municipality_profiles ADD COLUMN IF NOT EXISTS smout_sent_at timestamptz;
  ALTER TABLE municipality_profiles ADD COLUMN IF NOT EXISTS smout_reply text;
END;
$$;
