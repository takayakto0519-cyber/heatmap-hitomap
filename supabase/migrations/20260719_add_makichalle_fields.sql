-- biz_model_ideas にコンテスト応募専用フィールドを追加。
-- contest: どの応募案件に属するか（例：'makichalle2026'）。専用ダッシュボードの絞り込みに使う。
-- idea_no: 案の通し番号（①②③…の表示順）。
-- report_md: 1案ごとの詳細レポート（審査基準対応・二軸・仕組み・収益モデル・協賛候補などをMarkdownで）。
ALTER TABLE biz_model_ideas ADD COLUMN IF NOT EXISTS contest text;
ALTER TABLE biz_model_ideas ADD COLUMN IF NOT EXISTS idea_no integer;
ALTER TABLE biz_model_ideas ADD COLUMN IF NOT EXISTS report_md text;
CREATE INDEX IF NOT EXISTS idx_biz_model_ideas_contest ON biz_model_ideas (contest, idea_no);
