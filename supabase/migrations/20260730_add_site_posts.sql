-- 実績ブログ（過去のイベント紹介・参加者の感想）を運営ダッシュボードから書けるようにする site_posts。
-- あわせて 20260729 の ensure_schema 再定義で漏れていた appointment_requests を復帰させる。
CREATE TABLE IF NOT EXISTS site_posts (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  slug text UNIQUE NOT NULL,
  title text NOT NULL,
  category text NOT NULL DEFAULT 'event',   -- event（イベント実施）/ case（導入事例）/ note（お知らせ）
  event_date date,                          -- 実施日（並び順に使う）
  body text NOT NULL DEFAULT '',            -- 本文（空行区切りの段落）
  cover_url text,
  photo_urls text[] NOT NULL DEFAULT '{}',
  testimonials jsonb NOT NULL DEFAULT '[]', -- 参加者の感想 [{ name, comment }]
