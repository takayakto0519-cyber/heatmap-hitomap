-- コメントへのリアクション（いいね）。1人1コメントにつき1回まで。
...
CREATE POLICY "Users manage own comment reactions" ON comment_reactions FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
