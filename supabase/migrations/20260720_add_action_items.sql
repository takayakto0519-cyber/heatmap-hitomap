-- 作業状況ダッシュボード用テーブル（action_items）。
-- マキチャレ2026等、各案件の「次のアクション」を横断で一覧・状態管理するためのテーブル。
-- status: todo(未着手) | done(AI完了) | manual_required(会長作業待ち) | blocked(保留)
-- owner: AI | 会長 | 会長+小田
...
-- 【重要】このファイルは直前の20260719_add_municipality_profiles.sqlをベースに、action_itemsのみ追記している。
