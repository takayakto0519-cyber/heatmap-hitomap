-- AIエージェント運営タブ用テーブル（収益化イニシアチブ・案件パイプライン・顧問先カルテ・LINE設定・営業メール送り先）
-- これまでagents/配下のローカルMarkdown/JSONファイルで管理していたものを、運営ダッシュボードから直接編集できるようにする。
-- ローカルのエージェント（revenue_initiative_watch.py等）は当面ファイルを読み続けるため、
-- 完全な同期はまだ行っていない——このタブは「会長が手で見て・書き込める場所」として先に作る。
