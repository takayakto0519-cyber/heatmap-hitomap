-- サイト設定（site_settings）：トップページの大見出し・サブ文・ボタン文言、お知らせ帯など、
-- コード内に固定されていた「サイトの顔」の文言を運営ダッシュボードから編集できるようにする。
-- key ごとに jsonb で値を持つ（例：key='hero', value={eyebrow, headline_lines, subcopy, ...}）。
-- 値が無いキーはコード側の既定文言（lib/siteSettings.ts）で表示するため、空でもサイトは壊れない。
...
-- 注意：20260806版の ensure_schema は appointment_requests・site_posts・site_blocks・bonno系の定義が
-- 抜け落ちていた（新規環境で作られない状態）。ここで20260805版をベースに全定義を復元したうえで、
-- 20260806のbbox列・planカラムと今回の site_settings を追記する。
