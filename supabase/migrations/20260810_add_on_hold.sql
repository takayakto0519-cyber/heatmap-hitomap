-- municipality_profiles に「メール送信を保留にする」フラグを追加する。
-- 削除はせず一時的に営業対象から外す（休止）ためのもの。SalesTabの要フォロー通知・朝の一枚から除外する。
ALTER TABLE municipality_profiles ADD COLUMN IF NOT EXISTS on_hold boolean NOT NULL DEFAULT false;
