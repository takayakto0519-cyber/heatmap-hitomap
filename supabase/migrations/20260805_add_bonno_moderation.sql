-- 煩悩オークション(event_mode='bonno')の事前確認モード。
-- ONの投稿を持つイベントは、新規投稿がすぐ壁に出ず「pending」で溜まり、
-- 運営コンソール（/events/[slug]/console）で承認してから公開される。
-- 学校・法人向けにイベントを持ち込む前提で、デフォルトはOFF（従来通り即時公開）にしておく。
