-- お題つきクエストを運営ダッシュボードから管理できるようにする
CREATE TABLE IF NOT EXISTS quests (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  emoji text NOT NULL,
  title text NOT NULL,
  hint text NOT NULL,
  quest_type text NOT NULL DEFAULT 'search',   -- 'search'（探すお題） | 'emotion'（感情収集お題）
  target_emotion_key text,                      -- quest_type='emotion' のときの対象感情（lib/emotions.tsのkey）
