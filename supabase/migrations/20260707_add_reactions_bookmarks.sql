-- 共感ヒート（いいねの代わりに「わかる／歩いてみたい／懐かしい」の3種タップ）とブックマーク（お気に入り）

-- 1人1痕跡につき、種別ごとに1リアクション（複数種別を同時に付けられる）。重なるほどピンの色が濃く・大きくなる。
...
ALTER TABLE trace_reactions ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "Anyone can read reactions" ON trace_reactions;
CREATE POLICY "Anyone can read reactions" ON trace_reactions FOR SELECT USING (true);
DROP POLICY IF EXISTS "Users manage own reactions" ON trace_reactions;
CREATE POLICY "Users manage own reactions" ON trace_reactions FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

CREATE INDEX IF NOT EXISTS idx_trace_reactions_trace_id ON trace_reactions (trace_id);

-- 良い投稿をあとで見直すためのブックマーク
