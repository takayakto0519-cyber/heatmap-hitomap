-- 煩悩オークション：BONNO投資（参加者が共感した煩悩に持ち点を配分する仕組み）
-- 「本日の最高落札煩悩」は、この投資合計の降順で決まる（AI分析による切実さスコアは廃止）。
-- voter_token はブラウザ側でcrypto.randomUUID()により生成しlocalStorageに保持する匿名識別子。
-- RLS は有効化するがポリシーは作らない：読み書きはすべて Route Handler（service role）経由に限定する。
...
BEGIN
  ...
  -- 新規テーブル追加直後はPostgRESTのスキーマキャッシュが古いままになることがあるため、即時反映させる
