-- AIエージェント運営タブの追加分：事業別損益(P&L)テーブルと収益化イニシアチブの施策コード列。
-- 20260718_add_ai_ops_tables.sql の後に適用する。
...
-- /api/migrate から呼ばれる自動マイグレーション関数に反映。
-- 【重要】ensure_schema()はCREATE OR REPLACEで丸ごと再定義される。差分だけでなく、
-- 前回(20260718_add_ai_ops_tables.sql)の全定義をベースに今回分を追記すること（過去に全定義消失の事故あり）。
