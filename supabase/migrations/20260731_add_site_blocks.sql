-- 運営が自由に編集できるサイトコンテンツ（テキスト・画像・カード・引用・CTA）用の汎用ブロックテーブル。
-- ページ単位（page列）でブロックを sort_order 順に並べ、管理画面から追加・並び替え・非表示・削除できる。
CREATE TABLE IF NOT EXISTS site_blocks (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  page text NOT NULL DEFAULT 'home',
  sort_order integer NOT NULL DEFAULT 0,
  block_type text NOT NULL,          -- heading / text / image / cards / quote / cta
  eyebrow text,
  heading text,
  body text,
  image_url text,
  cta_label text,
  cta_href text,
  items jsonb NOT NULL DEFAULT '[]', -- cards: [{title,body,image_url,href,badge}] / quote: [{name,comment}]
