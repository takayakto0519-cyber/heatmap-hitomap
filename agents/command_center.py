...
# フロア名はlib/agents/roster.ts（単一の真実の源）のFLOORSが一次情報源。ここは表示用の簡易コピー。
# 2026-07-22: 137体を7部門AI（GA/SALES/PR/PRODUCT/CLIENT/FINANCE）に統合再編したのに合わせて更新。
FLOOR_NAMES = {
    "GA": "総務・内部運営部", "SALES": "営業・事業開発部", "PR": "マーケティング・広報部",
    "PRODUCT": "プロダクト運営部", "CLIENT": "顧問・人材部", "FINANCE": "財務部",
}

# agent_id -> (部門, 表示名, 注意判定に使うフィールド名 or None)
# urgent_fieldの値が0より大きければ「注意」とみなす。Noneは常時「正常」扱い（個別ロジックは_is_urgentで例外対応）。
AGENT_META: dict[str, tuple[str, str, str | None]] = {
    "approval_watch": ("GA", "送信待ちドラフトの監視", "stale_count"),
    "report_screen": ("PRODUCT", "通報一次スクリーニング", "pending_count"),
    "trace_qa": ("PRODUCT", "データ整合性QA", "issue_count"),
    "deadline_watch": ("GA", "課題締切トラッキング", "urgent_count"),
    "spam_detect": ("PRODUCT", "不正投稿検知", "duplicate_title_count"),
    "news_digest": ("PR", "ニュース抽出", None),
    "financial_snapshot": ("FINANCE", "財務・事業ダッシュボード要約", None),
    "case_pipeline_watch": ("SALES", "案件パイプライン", "stale_count"),
    "revenue_initiative_watch": ("SALES", "収益化イニシアチブ", "stale_count"),
    "office_diary": ("GA", "ビル日報", None),
    "lead_temperature": ("SALES", "リード温度感スコアリング", None),
    "payment_watch": ("FINANCE", "入金照合", "unpaid_count"),
    "lost_deal_archive": ("SALES", "失注理由アーカイブ", None),
    "schedule_watch": ("GA", "スケジュール", None),
    "line_mission": ("PRODUCT", "LINE縁ミッション", None),
    "email_queue": ("SALES", "営業メール下書きキュー", None),
    "trace_pattern": ("GA", "痕跡データパターン分析", None),
    "relation_population": ("SALES", "関係人口ダッシュボード", None),
    "calendar_watch": ("GA", "カレンダー", None),
    "competitor_market_research": ("PR", "競合・市場調査", None),
    "marketing_digest": ("PR", "マーケティング日報", None),
    "ab_test_summary_watch": ("GA", "UI改善A/Bテスト自動集計", None),
    "competitor_feature_monitor": ("GA", "競合プロダクト機能差分モニタ", "update_count"),
    "new_biz_signal_watch": ("SALES", "新規事業仮説の種探し", None),
    "global_market_watch": ("SALES", "海外展開リサーチ", None),
    "academic_partnership_watch": ("SALES", "産学連携リサーチ", None),
    "memorial_anniversary_watch": ("SALES", "周年史アーカイブ", "upcoming_count"),
    "action_items_digest": ("GA", "作業状況デイリー報告", "manual_required_count"),
}
