...
// 運営用サイトCMS：トップページのテキスト・画像・カード・引用・CTAを
// 自由に追加・編集・並び替え・非表示・削除できる管理画面。
...
export default function AdminBlocksPage() {
  ...
  async function load(pw = password, p = page) {
    const res = await fetch(`/api/admin/blocks?page=${p}`, { headers: { 'x-admin-password': pw } });
  ...
  useEffect(() => {
    ...
    if (saved) { setPassword(saved); load(saved, page); }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  useEffect(() => {
    if (authed) load(password, page);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [page]);

  async function seedHome() {
    setMessage('');
    const res = await fetch('/api/admin/blocks/seed-home', { method: 'POST', headers: headers() });
    const data = await res.json();
  ...
  async function move(id: string, dir: -1 | 1) {
    ...
    await fetch('/api/admin/blocks/reorder', {
      method: 'POST', headers: headers(), body: JSON.stringify({ order: next.map(b => b.id) }),
    });
  }

  async function toggleVisible(b: SiteBlock) {
    const res = await fetch(`/api/admin/blocks/${b.id}`, {
      method: 'PATCH', headers: headers(), body: JSON.stringify({ is_visible: !b.is_visible }),
    });
    const data = await res.json();
  ...
  async function remove(id: string) {
    ...
    const res = await fetch(`/api/admin/blocks/${id}`, { method: 'DELETE', headers: headers() });
    const data = await res.json();
  ...
  async function save() {
    ...
    try {
      ...
      const res = draft.id
        ? await fetch(`/api/admin/blocks/${draft.id}`, { method: 'PATCH', headers: headers(), body: JSON.stringify(payload) })
        : await fetch('/api/admin/blocks', { method: 'POST', headers: headers(), body: JSON.stringify(payload) });
      const data = await res.json();
  ...
  if (!authed) {
    return (
      <div style={{ maxWidth: 380, margin: '80px auto', padding: 24 }}>
        <h1 style={{ fontSize: 18, fontWeight: 800 }}>サイトCMS</h1>
  ...
  return (
    <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 20px 140px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', flexWrap: 'wrap', gap: 8 }}>
        <h1 style={{ fontSize: 20, fontWeight: 800, margin: 0 }}>サイトCMS</h1>
        <div style={{ display: 'flex', gap: 12, alignItems: 'baseline' }}>
          <a href="/" style={{ fontSize: 12, color: '#566246' }}>サイトを見る →</a>
          <a href="/admin/posts" style={{ fontSize: 12, color: '#888' }}>実績ブログ管理</a>
