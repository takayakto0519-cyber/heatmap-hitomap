...
export async function PATCH(req: NextRequest) {
  ...
  const body = await req.json().catch(() => ({})) as {
    display_name?: string; bio?: string; avatar_url?: string; character_name?: string;
  };
  ...
  if ('character_name' in body) updates.character_name = body.character_name?.trim().slice(0, 20) || null;
  const { data, error } = await supabase
