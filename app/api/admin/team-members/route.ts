...
export async function POST(req: NextRequest) {
  ...
  const body = await req.json().catch(() => ({})) as { name?: string; role?: string; is_lead?: boolean };
  if (!body.name?.trim()) return NextResponse.json({ ok: false, error: '名前は必須です' }, { status: 400 });

  const { supabaseServer } = await import('@/lib/supabase/server');
  const { data, error } = await supabaseServer
    .from('team_members')
    .insert({ name: body.name.trim(), role: body.role?.trim() || null, is_lead: Boolean(body.is_lead) })
    .select().single();

  if (error) {
    if (isMissingTable(error.message, 'team_members')) {
      return NextResponse.json({ ok: false, error: `運営メンバーのテーブルが未作成です。${MIGRATION_FILE} をSQL Editorで実行してください` }, { status: 503 });
    }
    return NextResponse.json({ ok: false, error: error.message }, { status: 500 });
  }
