...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { data: claimed, error: claimError } = await supabaseServer
    .from('municipality_profiles').update({ email_sent_at: new Date().toISOString() })
  ...
  try {
    ...
    await supabaseServer.from('municipality_profiles').update({ email_sent_at: null }).eq('id', params.id);
