#!/usr/bin/env node
// 公開日程調整サイト（Part B）用の、サーバー側Google Calendar認証情報を一度だけ発行するスクリプト。
// agents/calendar_watch.py・agents/gmail_watch.py と同じ hitomap-bot プロジェクトのOAuthクライアントを使う想定だが、
// あちらは「読み取り専用スコープ・ローカルファイル保存（会長のPCでしか動かない）」なのに対し、
// こちらは「読み取り＋書き込みスコープ・環境変数保存（Vercelの本番でも動く）」という別物として新規に作る。
//
...
//      テストユーザーとして登録済みであることを確認（calendar_watch.py設定時に済んでいるはず）。
//
// このスクリプトの実行（1回だけ・PowerShellで）：
//   node scripts/setup-google-calendar-oauth.mjs
...
//
// 新規npm依存は増やさない方針のため、Node標準機能（http, fetch）だけで実装している。
...
// 読み取り(freeBusy等)＋書き込み(events.insert)の両方をこの1回の許可でまとめて取る
const SCOPES = [
...
async function main() {
  ...
  if (!clientId || !clientSecret) {
    console.error([
      '',
      '❌ .env.local に GOOGLE_CALENDAR_CLIENT_ID / GOOGLE_CALENDAR_CLIENT_SECRET がありません。',
